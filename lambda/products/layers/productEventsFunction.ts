...
// Verificara todos os serviços aws
AWSXRay.captureAWS(require('aws-sdk'))

const eventsDdb = String(process.env.EVENTS_DDB)
...
function createEvent(event: ProductEvent) {
  ...
  // 5 minutos para apagar o dado
  const ttl = ~~(timestamp / 1000) + 5 * 60 // 5 minutos a frente : ~~ = arredonda o valor
